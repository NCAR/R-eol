...
plot.spectra <- function(x,log,multfreq=T,falloff,
    ...
    xaxt,yaxt,tlwd=par("lwd"),col,logo=T,
    ...) {
    ...
    if (any(!is.na(x@data[,1]))) {
        ...
        plot(x@frequencies,ytmp,log=logarg,type="n",
            xlab=xlab,ylab=ylab,
            xaxt=xaxt,yaxt=yaxt,
            xlim=frange,ylim=yrange,col=1,err=-1,...)
    }
...
plot_spectra_tics <- function(xs,ys,grid,logx,logy,col=1)
{
    ...
    if (logx) {
        ...
        for (i in pl) 
            mtext(substitute(10^i),side=1,line=par("mgp")[2]+0.2,at=10^i,adj=0.5)
    ...
    if (!missing(ys)) {
        if (logy) {
            ...
            for (i in pl)
                mtext(substitute(10^i),side=2,line=par("mgp")[2],at=10^i,adj=0.5,srt=90)
