...
plot.sunangle <- function(browse=F,year=2006,lat=40,lon=-105,yhi=90,twilight=T,localhours=T)
{
    ...
    cat("
        ...
        #----------------------------------------
        # Grab and Plot June, check az limits and plot the rest
        ...
        # Establish the nominal hour offset from greenwich
        # for longitude using 360/24 = 15 degree increments/hour
        # then lets create a string for the zones those represent.
        # no attempt to grab unix/dpar stuff for daylight, etc, just show
        # the standards and let the user be aware.
        # AST=Atlantic
        # AKST=Alaska
        # HAST=Hawaii/Aluetian
        # no attempt for the 'eastern zone names'
        TZoffset<- trunc(lon/15)
        TZones <- c("GMT","","","","AST","EST","CST","MST","PST","AKST","HAST")

        # Establish hourly data 
        # Using weeks instead of months to provide a more resolution
        nweeks <- 52
        gmt <- c(1:24)		# hours to plot....faster load

        hourelev <- array(0,c(24,nweeks+1))
        hourazm  <- array(0,c(24,nweeks+1))

        tlist <- list(year=year,mon=1,day=1,hour=0,min=0,sec=0,TZ="UTC")
        ut <- utime(tlist)
        ...
        for(i in c(1:nweeks)) {
            ...
            sun[!select,2] <- ylow
            hourazm[,i] <- sun@data[c(1:24),1]	# grab only 0Z-23Z values
        ...
        #
        hourelev[gmt,nweeks+1] <- hourelev[gmt,1]
        hourazm[ gmt,nweeks+1] <- hourazm[ gmt,1]
        ...
        for(i in seq(from=1,to=24)) {
            ...
            if( localhours ) {
                ...
                if( ix > 23 ) ix <- ix-24
                if( ix < 0  ) ix <- ix+24
                jdstamp <- paste(ix,sep="")	# hour for the calculated offset
            }
            else {
                # Or use this to label the GMT hours
                jdstamp <- paste(i-1,"Z",sep="")	# old Z method
            }
        ...
        if( localhours ) {
            text(300,yhi-5, paste(TZones[-TZoffset+1]," Hours; w/offset from gmt=",-TZoffset,sep=""))
            text(300,yhi-8, "Add 1 for DayLight Savings Time")
        }
        legend(c(xlow+4,xlow+80),c(yhi-12,yhi-3),c("Dec-Jun hours","Jun-Dec hours"),lty=c(5,6),col=c(2:3),cex=.8)
    ...
    #----------------------------------------
{
    ...
    cat("
        ...
        #----------------------------------------
        if( localhours ) {
            text(xlow*.5,ylow*.84, paste(TZones[-TZoffset+1]," Hours; w/offset from gmt=",-TZoffset,"\n Add 1 for Daylight Savings Time",sep=""))
        }
        text(xhi*.5,ylow*.8, "Shadow extends from 'center'
            ...
            yspan <- yspan*.07
            ylow <- ylow*.7
            legend(c(xlow-xspan,xlow),c(ylow,ylow+yspan),c("Dec-Jun","Jun-Dec"),lty=c(5,6),col=c(2,3),cex=.8)
