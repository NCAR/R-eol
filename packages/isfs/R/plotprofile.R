                        ...
                        hd, ...) 
{
    ...
    t1 <- positions(x)[1]
    tp1 <- utime((t1 %/% every) * every)
    if (tp1 < t1) tp1 <- tp1 + every

    # start times of profile plots
    tps <- seq(from=tp1, to=positions(x)[nrow(x)],by=every*ntraces)
    ...
    for (tp in tps) {
        ...
        for (tpx in seq(from=utime(tp),to=utime(te),by=every)) {
            tpx <- utime(tpx)
            it <- it + 1
