...
setMethod("readts",
    ...
    function(con,variables,start,end,...)
    {
        ...
        if (pid <= 0) {
            ...
            if (newversion) {
                runargs <- c("-D",paste(con@variables,collapse=","),
                    "-B",format(start,format="%Y %m %d %02H%02M%02S",TZ="GMT"),
                    "-C")
                env = ""
            }
            else {
                ops <- system(paste("getops",format(start,format="%Y %m %d %02H%02M%02S",TZ="GMT")),internal=TRUE)
            ...
            if (con@rate > 0.) runargs <- c(runargs,"-r",con@rate)
            ...
            if (!newversion) {
                ...
                units <- eval(parse(text=paste("c(",unix(com),")")))
            }
            else units = ""
        ...
        x <- .Call("read_prep",con,start,end,nrows,PACKAGE="isfs")

        stations(x) <- rep(0,ncol(x))
