    ...
    if (any(wvb == xh.name)) {
        ...
        if (!is.null(xh2o)) {
            ...
            if (!fcorr_done && !robust) {
                ...
                # no oxygen correction for IRGAs
                lo2corr <- xh2o * 0.0
                units(lo2corr) <- rep("",ncol(xh2o))
...
dat.Lv <- function(what,derived=TRUE,...)
{
    ...
    # [I haven not actually seen this book, but have a quotation]
...
dat.Scorr <- function(what,derived=TRUE,which="krypton",...)
{
    ...
    #
    # See Oncley et al., 2007 DOI 10.1007/s10546-007-9161-1, section 4.2.4 for a rough outline, with 
    # more details in Horst, 2003 (EGU paper).
    #
